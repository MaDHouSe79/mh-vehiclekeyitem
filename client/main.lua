...
local function CreateTempKey(vehicle)
    if vehicle then
        ...
        TriggerServerEvent('mh-vehiclekeyitem:server:CreateTempVehiclekey', {
            model = model,
            plate = plate
        })
    else
            ...
            QBCore.Functions.TriggerCallback('mh-vehiclekeyitem:server:GetOnlinePlayers', function(online)
                for k, vehicle in pairs(myVehicles) do
                    vehicleList[#vehicleList + 1] = {
                        value = vehicle.plate,
                        text = vehicle.vehicle .. " (" .. vehicle.plate .. ")"
                    }
                end
                ...
                if menu then
                    if not menu.id or not menu.vehicle then
                        ...
                        if vehicle then
                            TriggerServerEvent('mh-vehiclekeyitem:server:doAction', tonumber(menu.id),
                                tostring(menu.vehicle), vehicle)
                        else
...
RegisterNetEvent('mh-vehiclekeyitem:client:givekey', function(vehicle, plate)
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if pedvehicle == vehicle then
            ...
            TriggerServerEvent('mh-vehiclekeyitem:server:CreateNewVehiclekey', {
                model = displaytext,
                plate = plate
            })
        else
    ...
    if item and item.name == Config.KeyItem then
        ...
        if distance <= 2.5 then
            ...
            if item.info.plate == plate then
                ...
                TriggerServerEvent('mh-vehiclekeyitem:server:DeleteVehicleKey')
                QBCore.Functions.Notify(Lang:t('info.you_can_use_the_key', {
                    model = displaytext
                }), 'success')
            else
                QBCore.Functions.Notify(Lang:t('error.not_have_the_right_key', {
                    model = displaytext
                }), 'error')
            end
        else
